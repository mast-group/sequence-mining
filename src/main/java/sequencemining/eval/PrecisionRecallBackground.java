...
public class PrecisionRecallBackground {
	...
	public static void main(final String[] args) throws IOException, ClassNotFoundException {
		...
		// Set up logging
		final FileOutputStream outFile = new FileOutputStream(saveDir + "/" + name + "_pr.txt");
		...
		precisionRecall(sequences, "BIDE");
		precisionRecall(sequences, "ISM");

	}

	public static void precisionRecall(final Map<Sequence, Double> sequences, final String algorithm)
			throws IOException {
		...
		for (int k = 1; k <= minedSequences.size(); k++) {
			...
			final double rec = noInBoth / (double) sequences.size();
			precision[k - 1] = pr;
			recall[k - 1] = rec;
		}
